...
duplex.RPC = class RPC {
  ...
  registerFunc(method, func) {
    return this.register(method, ch =>
      ch.onrecv = (err, args) => func(args, (function(reply, more) { if (more == null) { more = false; } return ch.send(reply, more); }), ch)
    );
  }
    ...
    var connect = url => {
      ...
      this.ws.onopen = () => {
        return this.rpc.handshake(duplex.wrap.websocket(this.ws), p => {
          this.peer = p;
          return this.queued.map((args) =>
            (args => this.call(...args || []))(args));
        });
      };
