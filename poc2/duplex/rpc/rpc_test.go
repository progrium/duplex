...
func makePair(t *testing.T) (*Peer, *Peer) {
	client := NewPeer()
	server := NewPeer()
	err := server.Bind("inproc://rpc-test")
	if err != nil {
		t.Fatal(err)
	}
	err = client.Connect("inproc://rpc-test")
	if err != nil {
		t.Fatal(err)
	}
...
func TestAsyncCall(t *testing.T) {
	...
	addReply := new(Reply)
	addCall, _ := client.OpenCall(peer, "Arith.Add", args, addReply)
...
func (t *StreamingArith) Sum(channel *Channel) error {
	...
	channel.WriteObject(&StreamingReply{C: sum})
	return nil
}
...
func (t *StreamingArith) Echo(channel *Channel) error {
	...
	for {
		...
		channel.WriteObject(&StreamingReply{C: args.A, Index: args.Count})
	}
	return nil
}
